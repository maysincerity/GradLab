...
%token PLUS MINUS STAR DIV COMMENT ASSIGNOP AND OR NOT SELF_PLUS SELF_MINUS  IF ELSE WHILE FOR RETURN BREAK

%left ASSIGNOP PLUSASS MINUSASS
%left OR
%left AND
%left RELOP
%left PLUS MINUS SELF_PLUS SELF_MINUS
%left STAR DIV
%left COMMENT
%right UMINUS NOT
...
Stmt:   Exp SEMI    {$$=mknode(EXP_STMT,$1,NULL,NULL,yylineno);}
      ...
      | BREAK SEMI     {$$=mknode(BREAK,NULL,NULL,NULL,yylineno);$$->hasBreak=1;}
      ...
      | WHILE LP Exp RP Stmt {$$=mknode(WHILE,$3,$5,NULL,yylineno);$$->hasBreak=0;}
      ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      | Exp PLUSASS Exp {$$=mknode(PLUSASS,$1,$3,NULL,yylineno);strcpy($$->type_id,"PLUSASS");} //+=
      ...
      | Exp SELF_PLUS   {$$=mknode(SELF_PLUS,$1,NULL,NULL,yylineno);strcpy($$->type_id,"SELF_PLUS");} //自增
      | Exp SELF_MINUS   {$$=mknode(SELF_MINUS,$1,NULL,NULL,yylineno);strcpy($$->type_id,"SELF_MINUS");} //自减
      ...
      | ID LB INT RB  {$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);}
